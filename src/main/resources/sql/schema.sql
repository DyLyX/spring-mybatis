...
CREATE TABLE singer (
  ...
  `singId` bigint(11) NOT NULL COMMENT '歌手标识',
  `imageUrl` varchar(64) DEFAULT '' COMMENT '歌手头像',
  `type` tinyint(1) DEFAULT '0' COMMENT '类型：1男，2女，3组合',
  `createTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
   updateTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最后一次修改时间',
   PRIMARY KEY (`id`),
   UNIQUE KEY `UNIQUE` (`name`,`singId`)
